    ...
    <div class="container content">
        ...
        <div class="col-xl-12">
            ...
            <h4 class="title">
						...
						<form class="form-horizontal" method="post" action="/cristalensi/ricerca_avanzata_elenco.asp" name="newsform">
						<div class="col-lg-6">
		                ...
		                <div class="col-lg-9">
		                    <nav aria-label="Page navigation center-block">
		                        <ul class="pagination">
		                            ...
		                            <%if p > 2 then%>
		                            <li><a href="/cristalensi/ricerca_avanzata_elenco.asp?p=1&cat=<%=cat%>&FkProduttore=<%=FkProduttore%>&titolo=<%=titolo%>&prezzo_da=<%=prezzo_da%>&prezzo_a=<%=prezzo_a%>&order=<%=order%>">Prima pagina</a></li>
		                            <%end if%>
		                            <% if p > 1 then %>
		                            <li>
		                                <a href="/cristalensi/ricerca_avanzata_elenco.asp?p=<%=p-1%>&cat=<%=cat%>&FkProduttore=<%=FkProduttore%>&titolo=<%=titolo%>&prezzo_da=<%=prezzo_da%>&prezzo_a=<%=prezzo_a%>&order=<%=order%>" aria-label="Previous">
		                            ...
		                            <li><a href="/cristalensi/https://www.cristalensi.it/ricerca_avanzata_elenco.asp?p=<%=Page%>&cat=<%=cat%>&FkProduttore=<%=FkProduttore%>&titolo=<%=titolo%>&prezzo_da=<%=prezzo_da%>&prezzo_a=<%=prezzo_a%>&order=<%=order%>"><%=page%></a></li>
		                            <%end if%>
		                            ...
		                            <li>
		                                <a href="/cristalensi/ricerca_avanzata_elenco.asp?p=<%=p+1%>&cat=<%=cat%>&FkProduttore=<%=FkProduttore%>&titolo=<%=titolo%>&prezzo_da=<%=prezzo_da%>&prezzo_a=<%=prezzo_a%>&order=<%=order%>" aria-label="Next">
		                                    <span aria-hidden="true">&raquo;</span>
		                                </a>
		                            </li>
		                            <%end if%>
		                            <%if maxPage>5 and cInt(p)<>prod_rs.PageCount then%>
		                            <li><a href="/cristalensi/ricerca_avanzata_elenco.asp?p=<%=prod_rs.PageCount%>&cat=<%=cat%>&FkProduttore=<%=FkProduttore%>&titolo=<%=titolo%>&prezzo_da=<%=prezzo_da%>&prezzo_a=<%=prezzo_a%>&order=<%=order%>">Ultima pagina</a></li>
		                            <%end if%>
		                        </ul>
		                    </nav>
		                </div>
